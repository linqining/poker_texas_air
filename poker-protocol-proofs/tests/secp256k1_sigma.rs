...
#[test]
fn secp256k1_pk_ownership_roundtrip_and_rejection() {
...
#[test]
fn secp256k1_bg_shuffle_roundtrip_and_tamper_rejection() {
    let deck: Vec<BnPoint> = (0..8)
        .map(|i| Bn::hash_to_curve(format!("texas_poker_secp256k1/card/{i}").as_bytes()))
        .collect();
    let sk = random_scalar(&mut rand_core::OsRng);
    let pk = Bn::base_g() * &sk;
    ...
    let mut prove_transcript = transcript("secp256k1_bg_shuffle_v3");
    ...
    let mut verify_transcript = transcript("secp256k1_bg_shuffle_v3");
    proof
    ...
    let mut tamper_transcript = transcript("secp256k1_bg_shuffle_v3");
    ...
    let mut wrong_domain = transcript("secp256k1_bg_shuffle_v4");
...
fn secp256k1_bg_shuffle_full_deck_52() {
...
#[test]
fn secp256k1_fold_leave_dleq_roundtrip_and_tamper_rejection() {
    ...
    let mut prove_transcript = transcript("secp256k1_fold_leave_v3");
    ...
    let mut verify_transcript = transcript("secp256k1_fold_leave_v3");
    ...
    let mut remask_prove = transcript("secp256k1_remask_v3");
    ...
    let mut remask_verify = transcript("secp256k1_remask_v3");
    ...
    let mut cross = transcript("secp256k1_remask_v3");
    ...
    let mut tamper_verify = transcript("secp256k1_fold_leave_v3");
    assert!(!proof.verify(&input, &tampered, &pk, &mut tamper_verify));
}

#[test]
fn secp256k1_reveal_tokens_roundtrip_and_wrong_key_rejection() {
    ...
    for (ct, card) in ciphertexts.iter().zip(&deck) {
        ...
        let mut prove_transcript = transcript("secp256k1_reveal_token_v3");
        ...
        let mut verify_transcript = transcript("secp256k1_reveal_token_v3");
        ...
        let mut wrong_key_verify = transcript("secp256k1_reveal_token_v3");
        ...
        let mut forged_verify = transcript("secp256k1_reveal_token_v3");
        assert!(proof.verify(ct, &forged, &pk, &mut forged_verify).is_err());
    }
}

#[test]
fn secp256k1_three_player_hand_end_to_end() {
    ...
    let folded: Vec<BnCt> = deck_cts
        ...
        .collect();
    let mut fold_prove = transcript("secp256k1_fold_leave_v3");
    ...
    let mut fold_verify = transcript("secp256k1_fold_leave_v3");
    ...
    for ct in &deck_cts {
        ...
        for sk in [&sks[0], &sks[2]] {
            let token = ct.gen_reveal_token(sk);
            let mut token_prove = transcript("secp256k1_reveal_token_v3");
            let proof = RevealTokenProof::<Bn>::prove(
                sk,
                &(Bn::base_g() * sk),
                ct,
            ...
            let mut token_verify = transcript("secp256k1_reveal_token_v3");
            proof
