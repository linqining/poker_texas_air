...
pub fn run_and_adapt(
    ...
    let (program, mut hints, exec_scopes, context_builtins) = match program_type {
        ProgramType::Executable => {
            ...
            (program, hints, ExecutionScopes::new(), builtins)
        }
        ProgramType::Json => {
            let program = Program::from_bytes(&read(program_path)?, Some("main"))?;
            let json_builtins: Vec<_> = program.iter_builtins().cloned().collect();
    ...
    // Cairo0-style programs put start/stop pointers for all their header builtins
    // on the stack; executables only for their entrypoint builtins.
    let _ = context_builtins;
    ...
    match program_type {
        ProgramType::Executable => {
            let context = stwo_cairo_adapter::PublicSegmentContext::new(&context_builtins);
            stwo_cairo_adapter::adapter::adapt_with_context(&runner, context)
        }
        ProgramType::Json => adapt(&runner),
    }
}
