...
pub fn adapt(runner: &CairoRunner) -> Result<ProverInput> {
    adapt_with_context(runner, PublicSegmentContext::bootloader_context())
}

/// Adapts a run for proving, with an explicit public segment context (the set of
/// builtins whose start/stop pointers appear on the initial/final stack).
pub fn adapt_with_context(
    runner: &CairoRunner,
