...
//!
//! Poker-fork Merkle-compatible layout (see the constants below for the
//! authoritative offsets): the first four 256-bit slots are
//! `[cv, out_lo, msg_lo, msg_hi]`; the constant-wire pin sits at the tail.
//! ```text
//!   z[0      ..     256)       = cv[0..8]           slot 0
//!   z[256    ..     512)       = out_lo[0..8]       slot 1 (chain/merkle Z)
//!   z[512    ..    1,024)      = m[0..16]           slots 2..4 (X_L||X_R)
//!   z[1,024  ..    1,120)      = counter_lo/hi, block_len, flags
//!   z[1,152  ..   15,152)      = 56 G blocks × 250 bits each
//!   z[15,152 ..   15,408)      = out_hi[0..8] = state[8..16] ^ cv[0..8]
//!   z[15,408]                  = 1                  (constant-wire pin)
...
//
// **Poker fork (2026-08): Merkle-compatible re-layout.** The first four
// aligned 256-bit slots are `[cv, out_lo, msg_lo, msg_hi]` so the generic
// Merkle-path glue (`merkle_path_common`) sees the required contiguous
// 4-slot region: Z = out_lo (slot 1), X_L/X_R = the two message halves
// (slots 2/3), with the cv slot as the (weight-zero) "other" slot — the same
// geometry SHA-2's encoder was restructured for. The constant-wire pin
// `z[Z_CONST_POS] = 1` moved to the tail, one bit after `out_hi`.
...
pub const M_BASE: usize = 2 * SLOT_BITS; // 512, message slots 2..4
...
pub const GS_BASE: usize = FLAGS_BASE + WORD_BITS; // 1152
pub const OUT_HI_BASE: usize = GS_BASE + N_G * G_STRIDE; // 15,152
pub const Z_CONST_POS: usize = OUT_HI_BASE + 8 * WORD_BITS; // 15,408, pin at the tail
pub const USEFUL_BITS: usize = Z_CONST_POS + 1; // 15,409, pin bit is useful (non-zero)
...
impl Blake3Setup {
    ...
    /// Prove a Merkle path of length `K = 2^n_log` BLAKE3 compressions
    /// (poker fork). The prover supplies one `Compression` per internal node:
    /// `node_{i+1} = out_lo(blake3_compress(cv, X_L||X_R, ...))` where the
    /// `b_bits[i]`-selected message half equals `out_lo` of compression
    /// `i-1`, and the first compression's selected half equals the public
    /// leaf. `n_blocks` must exactly fill `n_block_slots` (power of 2 ≥ 8).
    ///
    /// The counter/block_len/flags fields are prover-witness: soundness of
    /// the leaf→root binding rests on BLAKE3 compression preimage
    /// resistance, not on flag discipline.
    pub fn prove_merkle_path<Ch: Challenger>(
        &self,
        compressions: &[Compression],
        b_bits: &[bool],
        challenger: &mut Ch,
    ) -> (
        super::merkle_path_common::MerklePathProofLigerito,
        Commitment,
    ) {
        assert_eq!(compressions.len(), self.n_blocks);
        assert_eq!(self.n_blocks, self.n_block_slots());
        assert_eq!(b_bits.len(), self.n_block_slots());
    ...
    /// Verifier for [`Self::prove_merkle_path`]. `leaf`/`root` are the public
    /// 256-bit endpoints in the same LE-word encoding as `cv_to_phys_bits`.
    pub fn verify_merkle_path<Ch: Challenger>(
        &self,
        commitment: &Commitment,
        proof: &super::merkle_path_common::MerklePathProofLigerito,
        leaf: &[u32; 8],
        root: &[u32; 8],
        b_bits: &[bool],
        challenger: &mut Ch,
    ) -> Result<(), super::merkle_path_common::MerklePathVerifyError> {
        assert_eq!(self.n_blocks, self.n_block_slots());
        assert_eq!(b_bits.len(), self.n_block_slots());
...
// ---------------------------------------------------------------------------
// Merkle-path geometry (poker fork)
// ---------------------------------------------------------------------------

/// Merkle-path region for the re-layouted witness: the first four aligned
/// 256-bit slots are `[cv, out_lo, msg_lo, msg_hi]`, so Z = `out_lo`
/// (slot 1), X_L/X_R = the two message halves (slots 2/3), and the `cv`
/// slot is the weight-invisible "other" slot.
...
mod tests {
    ...
    /// Poker-fork Merkle-path roundtrip: a depth-256 path (the production
    /// shape) of `node_{i+1} = out_lo(compress(IV, X_L||X_R, 0, 64, 0))`
    /// climbing from a public leaf to a public root, with public direction
    /// bits.
    #[test]
    fn merkle_path_roundtrip_depth256() {
        ...
        let mut compressions: Vec<Compression> = Vec::with_capacity(depth);
        let mut node = leaf;
        for i in 0..depth {
            let (x_l, x_r) = if b_bits[i] {
                (siblings[i], node)
            } else {
                (node, siblings[i])
            };
            let mut m = [0u32; 16];
            m[0..8].copy_from_slice(&x_l);
            m[8..16].copy_from_slice(&x_r);
            let st = blake3_compress(&BLAKE3_IV, &m, 0, 64, 0);
            compressions.push((BLAKE3_IV, m, 0u64, 64u32, 0u32));
            node = st[0..8].try_into().unwrap();
        }
        let root = node;

        let setup = Blake3Setup::new(depth);
        let mut ch_p = FsChallenger::new(b"flock-blake3-merkle-v0");
        let (proof, commitment) = setup.prove_merkle_path(&compressions, &b_bits, &mut ch_p);
        let mut ch_v = FsChallenger::new(b"flock-blake3-merkle-v0");
        setup
            .verify_merkle_path(&commitment, &proof, &leaf, &root, &b_bits, &mut ch_v)
            .expect("honest merkle path verifies");

        // Wrong root / wrong leaf / flipped direction bit must all reject.
        let mut ch_v = FsChallenger::new(b"flock-blake3-merkle-v0");
    ...
    /// Poker-fork Merkle-path roundtrip with a periodic direction-bit
    /// pattern.  NOTE: the shift protocol convention requires the path
    /// start in the X_L slot, i.e. `b_bits[0] = false` — a leading-true bit
    /// is fail-closed (SumcheckFinal), so callers must place the path start
    /// on the left of the level-0 message.
    #[test]
    fn merkle_path_roundtrip_periodic_bits() {
        use flock_core::challenger::FsChallenger;

        let depth = 256usize;
        let b_bits: Vec<bool> = (0..depth).map(|i| i % 8 == 4).collect();
        let mut leaf: [u32; 8] = [
        ...
        let siblings: Vec<[u32; 8]> = (0..depth)
            .map(|h| {
                let mut s = [0x5a5a5a5au32; 8];
                s[0] = 0x5a5a5a00 | (h as u32);
                s
            })
            .collect();
    ...
    fn layout_constants() {
        ...
        // message halves in slots 2/3 (Merkle-path compatible).
        assert_eq!(CV_BASE, 0);
        assert_eq!(OUT_LO_BASE, 256);
        assert_eq!(M_BASE, 512);
        assert_eq!(GS_BASE, 1152);
        assert_eq!(G_STRIDE, 250);
        assert_eq!(N_G, 56);
        assert_eq!(OUT_HI_BASE, 15_152);
