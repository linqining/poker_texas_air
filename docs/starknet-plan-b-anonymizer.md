...
3. ~~提现（cash-out）目前是公开 `vault.withdraw`~~ **已完成（TODO #25，2026-09-03）**：vault `withdraw_to` + `CashoutUnshieldHelper`（chip_to_note）已实现（snforge 覆盖）并随 v3 批次部署 sepolia；池侧 note 应用入口仍等官方 SDK（#12/#26）。
